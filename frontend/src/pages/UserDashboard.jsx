...
const UserDashboard = () => {
  ...
  const fetchRecommendations = async () => {
    try {
      const { data } = await api.get("/bookings/recommendations");
      ...
      {recommendations.length > 0 && (
        <section className="mt-14">
          <div className="mb-6">
            ...
            <h2 className="mt-1 text-2xl font-extrabold text-[#11162f]">
              Recommended Events ⭐
            </h2>
            ...
            {recommendations.map((event) => (
              ...
              >
                {event.image ? (
                  ...
                  <div className="flex h-44 items-center justify-center bg-purple-100 font-bold text-purple-600">
                    {event.category}
                  </div>
                )}
                ...
                <div className="p-5">
                  ...
                  <Link
                    to={`/events/${event._id}`}
                    className="mt-4 block rounded-xl bg-purple-600 py-3 text-center font-bold text-white hover:bg-purple-700"
                  >
                    View Event
                  </Link>
                </div>
                  ...
                  <>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex flex-col gap-1 items-end">
                        ...
                        {/* View Ticket Link (Only for Confirmed Bookings) */}
                        {booking.status === "confirmed" && (
                          <Link
                            to={`/ticket/${booking._id}`}
                            className="inline-flex items-center gap-1 font-semibold text-purple-600 transition hover:text-purple-800"
                          >
                            🎟️ View Ticket
                          </Link>
                        )}
